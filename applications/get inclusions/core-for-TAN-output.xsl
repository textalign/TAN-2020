...
<xsl:stylesheet xmlns:tan="tag:textalign.net,2015:ns" xmlns="tag:textalign.net,2015:ns"
   ...
   <xsl:template match="/tan:* | /tei:TEI" mode="credit-stylesheet">
      ...
      <xsl:copy>
         ...
         <xsl:apply-templates mode="#current">
            ...
            <xsl:with-param name="stylesheet-id" tunnel="yes" select="$stylesheet-id"/>
   ...
   <xsl:template match="tan:resp[@roles = 'stylesheet'][1]/@who" mode="credit-stylesheet">
      <xsl:param name="stylesheet-id" tunnel="yes" as="xs:string?"/>
      <xsl:choose>
         <xsl:when test="contains-token(., $stylesheet-id)">
            <xsl:copy-of select="."/>
         </xsl:when>
         <xsl:otherwise>
            <xsl:attribute name="who" select="string-join((., $stylesheet-id), ' ')"/>
         </xsl:otherwise>
      </xsl:choose>
   </xsl:template>
   ...
   <xsl:template match="tan:body" mode="credit-stylesheet">
      ...
      <xsl:copy>
         ...
         <xsl:if test="tan:class-number(.) = 2">
            ...
            <xsl:attribute name="claimant" select="$stylesheet-id"/>
         </xsl:if>
         <xsl:apply-templates mode="#current"/>
      </xsl:copy>
   </xsl:template>
