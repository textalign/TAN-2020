<xsl:stylesheet
   ...
   <xsl:template match="tan:div | tei:div" mode="tan:core-expansion-terse">
      ...
      <div>
         ...
         <!-- space normalization has already occurred, so the items below are commented out -->
   ...
   <xsl:template match="tei:lb | tei:pb | tei:cb"
      ...
      <xsl:copy>
         ...
         <xsl:if test="not($text-should-be-joined) and not($element-has-adjacent-space)">
            <xsl:copy-of select="tan:error('tei05', ('prev text: [' || $prev-text-joined || ']; next text: [' || $next-text-joined || ']'))"/>
         </xsl:if>
